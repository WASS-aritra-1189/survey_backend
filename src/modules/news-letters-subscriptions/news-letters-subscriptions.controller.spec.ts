/**
 * Copyright (c) 2025 Webapp Software Solutions. All rights reserved.
 * This file belongs to Webapp Software Solutions and is proprietary and confidential.
 * Unauthorized copying of this file, via any medium is strictly prohibited.
 * Website: https://webappssoft.com
 */

import { Test, TestingModule } from '@nestjs/testing';
import { NewsLettersSubscriptionsController } from './news-letters-subscriptions.controller';
import { NewsLettersSubscriptionsService } from './news-letters-subscriptions.service';

describe('NewsLettersSubscriptionsController', () => {
  let controller: NewsLettersSubscriptionsController;

  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      controllers: [NewsLettersSubscriptionsController],
      providers: [NewsLettersSubscriptionsService],
    }).compile();

    controller = module.get<NewsLettersSubscriptionsController>(NewsLettersSubscriptionsController);
  });

  it('should be defined', () => {
    expect(controller).toBeDefined();
  });
});
